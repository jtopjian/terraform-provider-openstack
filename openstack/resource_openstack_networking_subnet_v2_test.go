		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2SubnetHostRouteCount(&subnet, 1),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					testAccCheckNetworkingV2SubnetHostRouteCount(&subnet, 2),
				),
			},
		},
	})
...
func testAccCheckNetworkingV2SubnetHostRouteCount(subnet *subnets.Subnet, num int) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		if len(subnet.HostRoutes) != num {
			return fmt.Errorf("Unexpected number of routes: wanted %d, got %d",
				num, len(subnet.HostRoutes))
		}
